...
class ContentAnalyzer:
    ...
    async def analyze_file(self, file_path: str, content_type: str):
        ...
        try:
            ...
            try:
                ...
                for field, expected_type in self.required_fields.items():
                    if field not in metadata:
                        ...
                        if isinstance(expected_type, tuple):
                            if not any(
                                isinstance(metadata[field], t) for t in expected_type
                            ):
                                self.missing_required[content_type][
                                    f"{field}_type"
                                ].append(file_path)
                        else:
                            self.missing_required[content_type][f"{field}_type"].append(
                                file_path
                            )
                ...
                seo_suggestions = await self._get_seo_suggestions(
                ...
                self.missing_required[content_type]["invalid_front_matter"].append(
                    file_path
                )
        ...
        with Progress(
            ...
            for content_dir in glob.glob(f"{content_config.content_dir}/*"):
                ...
                task_id = progress.add_task(
                    f"Analyzing {content_type} content...", total=None
                )
    ...
    def print_report(self):
        ...
        for content_type, total in self.total_files.items():
            invalid = (
                len(self.missing_required[content_type].get("front_matter", []))
                + len(
                    self.missing_required[content_type].get("invalid_front_matter", [])
                )
        ...
        for metric_name, (metric_path, threshold) in metrics.items():
            ...
            if count > 0:
                ...
                quality_table.add_row(metric_name, str(avg_score), str(below_threshold))
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze content quality and structure"
    )
