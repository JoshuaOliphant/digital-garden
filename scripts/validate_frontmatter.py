...
class ContentValidator:
    ...
    def validate_links(self, content: str) -> List[Dict[str, any]]:
        ...
        for text, url in links:
            try:
                ...
                results.append(
                    {"text": text, "url": url, "valid": False, "error": str(e)}
                )
    ...
    def check_accessibility(self, content: str) -> Dict[str, any]:
        ...
        for img in images:
            if not img.get("alt"):
                issues.append(
                    f"Missing alt text for image: {img.get('src', 'unknown')}"
                )
...
class FrontMatterValidator:
        ...
        if "tags" in fixed:
            ...
            if "title" in fixed:
                ...
                title_tags = [
                    ...
                    and word not in {"notes", "the", "and", "for", "with"}
                ]  # Skip common words
    ...
    def validate_file(
        self, file_path: str, content_type: str, fix: bool = False
    ) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            try:
                ...
                if len(parts) != 2:
                    return False, [
                        "Invalid front matter format - missing closing '---'"
                    ]
                ...
                try:
                    ...
                    validation_results = {
                        "writing_style": self.content_validator.check_writing_style(
                            md_content
                        ),
                        "links": self.content_validator.validate_links(md_content),
                        "accessibility": self.content_validator.check_accessibility(
                            md_content
                        ),
                    }
                    ...
                    if validation_results["writing_style"]["style_score"] < 7:
                        issues.extend(
                            validation_results["writing_style"]["improvements"]
                        )

                    # Link issues
                    invalid_links = [
                        link
                        for link in validation_results["links"]
                        if not link["valid"]
                    ]
                    if invalid_links:
                        issues.extend(
                            [
                                f"Invalid link: {link['url']} - {link.get('error', link.get('status_code'))}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate front matter and content in files"
    )
